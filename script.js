...
function setTime() {
    ...
    const hoursForClock = hours >= 13 ? hours % 12 : hours;   // ? Mark denoted by if and : denoted by else
    const minute = time.getMinutes();
    const seconds = time.getSeconds();
    const ampm = hours >= 12 ? 'PM' : 'AM'

    hourEl.style.transform = `translate(-50%, -100%)rotate(${scale(hoursForClock, 0, 11, 0, 359)}deg)`
    minuteEl.style.transform = `translate(-50%, -100%)rotate(${scale(minute, 0, 59, 0, 359)}deg)`
